...
struct ThreadManager {
    ...
    int nested_threads;          // Number of threads for nested parallel regions

    // Default constructor
    ThreadManager()
        : harmonic_bond_threads(1), lennard_jones_threads(1), electrostatic_threads(1), nested_threads(1) {}

    // Constructor with initial values
    ThreadManager(int hb_threads, int lj_threads, int es_threads, int n_threads)
        : harmonic_bond_threads(hb_threads), lennard_jones_threads(lj_threads), electrostatic_threads(es_threads),
            nested_threads(n_threads) {}
};
