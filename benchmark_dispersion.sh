...
D_VALUES=(0 1 2 4 8 12 16 32 64)                      # Values for the -e flag (threads)
N_VALUES=(1000 5000 10000)                 # Values for the -n flag (size)
REPEATS=3                                 # Number of repetitions per configuration

# Initialize CSV file with headers
echo -n "Size/Threads" > $OUTPUT_FILE
for e in "${D_VALUES[@]}"; do
...
for n in "${N_VALUES[@]}"; do
  ...
  for e in "${D_VALUES[@]}"; do
    ...
    # perf_output=$(/usr/lib/linux-tools/5.15.0-126-generic/perf stat -e cache-misses,cache-references --repeat $REPEATS $EXECUTABLE -n $n -s 1 -d $e 2>&1)
    perf_output=$(perf stat -e cache-misses,cache-references --repeat $REPEATS $EXECUTABLE -n $n -s 1 -d $e 2>&1)
